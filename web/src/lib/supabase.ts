...
export async function getNewlyEarnedMedals(_monumentId: string): Promise<EarnedMedal[]> {
  ...
  return data
    .map((r: Record<string, unknown>) => {
      const medal = r.medals as Omit<EarnedMedal, "earned_at"> | null;
      return medal ? { ...medal, earned_at: r.earned_at as string } : null;
    })
    .filter((medal): medal is EarnedMedal => medal !== null);
}
...
export async function getPublicProfile(userId: string) {
  const { data, error } = await supabase
    ...
    .single();
  if (error || !data) return null;
  return data as { id: string; display_name: string | null; avatar_url: string | null; bio: string | null; is_public: boolean };
}
...
export async function getCollectionMonuments(collectionId: string, limit = 4): Promise<CollectionMonument[]> {
  const { data, error } = await supabase.rpc("get_collection_monuments", {
    p_collection_id: collectionId,
    p_limit: limit,
  });
  if (error) return [];
  return (data ?? []) as CollectionMonument[];
}
